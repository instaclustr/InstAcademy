← [InstAcademy](README.md) · [Contributing](CONTRIBUTING.md) · [Report a problem](https://github.com/instaclustr/instacademy/issues/new/choose)

# Security

InstAcademy courses are teaching material, not a deployed service, so the risks here are mostly about **credentials leaking into the repository** and about learners copying lab shortcuts into production. Both are avoidable.
...
If you find a security problem in the course material itself (a committed credential, an instruction that would expose a cluster), please [open an issue](https://github.com/instaclustr/instacademy/issues/new/choose) if it is not sensitive, or contact the InstAcademy maintainers privately if it is.
