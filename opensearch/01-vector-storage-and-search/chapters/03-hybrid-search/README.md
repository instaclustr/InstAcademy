← **Previous:** [Chapter 2](../02-neural-search-pipelines/README.md) · [Course index](../../README.md) · [How to run labs](../../HANDS-ON-GUIDE.md) · **Next:** [Chapter 4](../04-rag-optimization/README.md) →
...
```json
{
  ...
  "create_time": 1783954098444,
  "last_update_time": 1783954207742,
  "is_async": true
}
```
...
```json
{
  ...
  "create_time": 1783954389076,
  "last_update_time": 1783954441923,
  "is_async": true
}
```
...
GET my-sparse-neural-index/_search?filter_path=hits.hits._id,hits.hits._score,hits.hits._source,hits.hits.highlight
{
  ...
  "highlight": {
    "fields": { "passage_text": { "fragment_size": 120, "number_of_fragments": 1 } }
  }
}
```
    ...
    "hits": [
      {
        ...
        "highlight": {
          "passage_text": [
            "<em>voyage</em> to colonial servitude."
          ]
        }
      },
      {
        ...
        "highlight": {
          "passage_text": [
            "\"Twenty Thousand Leagues under the <em>Sea</em>\" by Jules Verne is <em>a</em> science fiction adventure novel serialized from 1869 to 1870."
          ]
        }
      },[...]
```

**Save** the **top-5 titles and order** — you will compare the results of sparse, dense, hybrid, and RRF.
...
GET my-sparse-neural-index/_search?filter_path=hits.hits._id,hits.hits._score,hits.hits._source,hits.hits.highlight
{
  ...
  "highlight": {
    "fields": { "passage_text": { "fragment_size": 120, "number_of_fragments": 1 } },
    "highlight_query": { "match": { "passage_text": "a hero on a dangerous sea voyage" } }
  }
}
```
...
- **The arrivals (bolded) are the semantic wins.** *Gulliver's Travels* hides the best detail in the whole response: its highlight reads "embarks on extraordinary voyages," and *voyages* is **not** wrapped in `<em>`. That's not a highlighting glitch, it's the explanation. The analyzer doesn't stem, so the token `voyages` never matched your query's `voyage`, which is exactly why BM25 missed this book. The sparse model knows the two words mean the same thing. *Four Arthurian Romances* is the purer case: "a quest that leads him through various perils" shares no meaningful word with the query at all, yet it's a near-paraphrase of "a hero on a dangerous sea voyage."
- **The departure is the noise leaving.** *Don Quijote* only made the lexical list on stopword overlap (its Step 7 highlight showed little more than "on"). Scored by meaning, it's gone.
- **The demotion is a re-weighting.** *Kidnapped* fell from #1 to #5: it earned lexical first place on one literal word ("voyage"), but on overall meaning it's a kidnapping story with a sea leg, not a sea-voyage story.

A note on the scores. You might notice the sparse scores (7 to 12) are much bigger than the BM25 scores (2 to 4). That does **not** mean sparse found "better" matches; the two methods calculate scores with completely different formulas, so their numbers aren't comparable. When comparing methods, only the *order* of results matters. Score values are only meaningful within a single result list, and there they do tell you something: the top two sparse hits scored 11+, while the next three sit around 7.9. That gap means the model rates the two true sea-voyage stories as much stronger matches than the rest, not merely one and two places higher.
  ...
  "hits": {
    "hits": [
      ...
      },
      {
        ...
        "highlight": {
          "passage_text": [
            "When young Jim Hawkins discovers <em>a</em> mysterious treasure map in <em>a</em> dead pirate's <em>sea</em> chest, he sets sail with <em>a</em> crew to find"
          ]
        }
      }[...]
    ]
...
- **Dense agrees with sparse on the semantic catch (*Gulliver's Travels*) but drifts further.** *Life on the Mississippi* (a riverboat memoir) and *Undine* (a water-sprite fairy tale) are here because dense embeddings match broad theme, and "watery journey" is thematically adjacent even though neither is a hero-on-a-dangerous-voyage story. Meanwhile *Twenty Thousand Leagues*, the #1 or #2 pick of both other methods, misses the dense top-5 entirely. Dense casts the widest net and pays for it with the loosest catch.
- **The dense scores are tiny and nearly tied** (0.0154 to 0.0161, a spread of about 4%). This is the same compressed L2 score band you saw in Chapter 2, and here it means dense is barely discriminating among its top picks on this corpus.

The takeaway for the rest of the chapter: no single method got this query "right." Lexical found literal matches plus a stopword freeloader, sparse found meaning but demoted a literal match, dense found theme but wandered. Each list contains books the others missed and noise the others avoided. Merging them so the strengths cover the weaknesses is exactly what Lessons 3-2 and 3-3 do, two different ways.

**Save** the table above. Lessons 3-2 and 3-3 line the fused rankings up against it.

**Fast mode** `bruno/03-hybrid-search/14-dense-search-compare.bru`
...
3. Kidnapped (~0.46)
4. Robinson Crusoe (~0.23)
5. Gulliver's Travels (~0.20)

Hold this against your Step 7 and Step 8 lists and the fusion logic becomes visible:

- **Consensus takes the podium.** The three books that appeared on *both* single-method lists (Twenty Thousand Leagues, Treasure Island, Kidnapped) now hold the top three spots. Agreement between methods is rewarded automatically.
- **Each method contributes a save.** *Robinson Crusoe* (lexical-only, sparse dropped it) is pulled back in, and *Gulliver's Travels* (sparse-only, invisible to keywords) keeps its seat. That is hybrid's whole pitch in one row each: the strengths of each branch cover the other's blind spots.
...
![Reading the fused score: stacked contributions per book](../../assets/03-hybrid-search/diagram-06-fused-score-stacked-contributions.png)

**Save** the **top-5 order**; you will compare it against the RRF ranking in Lesson 3-3.

**Fast mode** `bruno/03-hybrid-search/16-hybrid-normalized-search.bru`
...
RRF is the escape from all of that. It throws the scores away entirely and asks each branch a single, scale-free question: *where did you rank this document?* A book that both branches place near the top accumulates a big fused score no matter what raw numbers the branches produced. Remember the observation from your Step 9 table, that the one book appearing on all lists (*Treasure Island*) was probably the most trustworthy result? RRF is that intuition turned into a formula.

In this lesson you'll build the RRF pipeline, run the *identical* hybrid query through it, and compare the rank-fused list against the score-fused one, closing out the chapter with all five rankings side by side.
...
3. Kidnapped (~0.0229)
4. Robinson Crusoe (~0.0216)
5. Gulliver's Travels (~0.0163)

Same five books as the normalized run, in the same order — and that convergence is itself the finding: with the branches and weights held constant, two completely different fusion strategies agreed on the answer. When score fusion and rank fusion produce the same list, you can trust that the ranking comes from genuine agreement between the branches, not from a quirk of either formula.

What *does* differ is the shape of the scores, and RRF's are fully auditable in a way normalized scores never quite were. Every RRF score is just `Σ weight / (k + rank)`. Check your own output to see their scores.
...
**Fast mode** `bruno/03-hybrid-search/18-hybrid-rrf-search.bru`

---

Take a second to appreciate what your five-column table represents. You ran one query five ways and watched three imperfect methods, each wrong in its own style, get fused into two rankings that kept every method's best find and none of its noise. Along the way you worked with a second kind of embedding model, put chunking into a real pipeline, learned why chunk size can make or break ranking quality, and proved that fusion behavior lives entirely in a search pipeline you can swap without touching a query or reindexing a document. Most engineers ship hybrid search by copying a config they've never interrogated; you just built both fusion strategies from parts and audited their scores by hand.

## ✅ What you learned

- **Why hybrid wins:** lexical, dense, and sparse each rank the same query differently; fusing them recovers precision *and* recall.
- **Neural sparse** encodings (`rank_features` + `nested`) bridge lexical and neural.
...
**Fast mode** `bruno/03-hybrid-search/19-cleanup.bru` (sends the first DELETE; its docs note lists the other three requests to run by hand).

### Chapter 2 cleanup (deferred until now)

Delete Chapter 2's search pipeline, index, and ingest pipeline:
...
Undeploy the dense model (a deployed model cannot be deleted), then poll `GET _plugins/_ml/tasks/YOUR_TASK_ID` until the state is terminal:

```http
POST _plugins/_ml/models/YOUR_DENSE_MODEL_ID/_undeploy
```
...
Everything so far has been about getting the right documents to the top of a list for a *person* to read. Chapter 4 changes the customer: the results now feed an **AI system**. You'll build a production RAG retrieval layer for a bookstore: chunked at ingest, filtered by metadata, hybrid-searched, reranked by business signals, and measured with an actual relevance metric instead of eyeballing, then cap it off by exposing the whole thing to AI agents through OpenSearch's built-in MCP server, so an LLM can discover your index and query it on its own. The hybrid pipeline you just mastered is about to become one component in a bigger machine.

[Chapter 4 · Lesson 1](../04-rag-optimization/README.md) — bookstore **RAG** with dense k-NN and hybrid search.

---

🚀 **Next:** [Chapter 4](../04-rag-optimization/README.md)
