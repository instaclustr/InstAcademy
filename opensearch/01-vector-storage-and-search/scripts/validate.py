...
CHAPTER_SLUGS = [
    "01-configuring-vector-search",
    "02-neural-search-pipelines",
    "03-hybrid-search",
    "04-rag-optimization",
    "05-production-optimizations",
]
CHAPTERS = range(1, len(CHAPTER_SLUGS) + 1)


def chapter_dir(ch):
    return f"chapters/{CHAPTER_SLUGS[ch - 1]}"


def bruno_dir(ch):
    return f"bruno/{CHAPTER_SLUGS[ch - 1]}"
...
def check_bruno():
    ...
    for ch in CHAPTERS:
        readme_path = f"{chapter_dir(ch)}/README.md"
        folder = bruno_dir(ch)
...
def check_bulk():
    ...
    for ch in CHAPTERS:
        ...
        for f in sorted(os.listdir(folder)):
            if not f.endswith(".bru"):
                continue
            content = open(os.path.join(folder, f)).read()
            # Bruno resolves @file() from the COLLECTION ROOT (the dir holding
            # bruno.json), not from the .bru file's own folder.
            m = re.search(r"file:\s*@file\(([^)]*)\)", content)
            if not m:
                continue
            target = os.path.normpath(os.path.join("bruno", m.group(1).strip()))
    ...
    readme_blocks = []
    for ch in CHAPTERS:
        text = open(f"{chapter_dir(ch)}/README.md").read()
